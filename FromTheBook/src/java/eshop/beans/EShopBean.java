...
public class EShopBean {
    ...
    public int saveOrder( String city , String state , String street , String number , String zip ,
                          String country , String phone , String fax , List<String> shopping_cart ,
                          double payment ) {

        String[] customers = { "tom@yahoo.com" ,
                               "john@yahoo.com" ,
                               "marian@yahoo.com" };
        String[] passwords = { "tomandrafa" ,
                               "johnandrafa" ,
                               "marianandrafa" };
        ...
        if ( customer != null ) {
            for ( String product : shopping_cart ) {
                ...
                if ( st.hasMoreTokens() ) {
                    ...
                    token = st.nextToken();
                    cart_product.setName(
                            token.substring( token.indexOf( ":" ) + 1 , token.length() ).trim() );
                    token = st.nextToken();
                    cart_product.setColor( token.substring( token.indexOf( ":" ) + 1 ,
                                                            token.length() ).trim() );
                    token = st.nextToken();
                    cart_product.setSize(
                            token.substring( token.indexOf( ":" ) + 1 , token.length() ).trim() );
                    token = st.nextToken();
                    cart_product.setPrice( Double.valueOf(
                            token.substring( token.indexOf( ":" ) + 1 , token.length() ).trim() ) );
                    token = st.nextToken();
                    cart_product.setQuantity( token.substring( token.indexOf( ":" ) + 1 ,
                                                               token.length() ).trim() );
                }
